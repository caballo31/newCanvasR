...
import { Folder, X, Copy, Edit } from 'lucide-react'
    ...
    >
      ...
      {isSelected && (
        <div className="absolute -top-8 left-1/2 transform -translate-x-1/2 flex gap-1 bg-gray-800 rounded-md p-1 shadow-lg">
          ...
          <button
            onClick={(e) => {
              e.stopPropagation()
              const newContent = prompt('Editar nombre de carpeta:', node.content)
              if (newContent !== null) {
      ...
      <div className="w-full h-full flex items-center justify-center p-4 bg-gray-50 rounded-lg">
        <div className="text-center">
          <Folder size={24} className="text-gray-400 mb-2 mx-auto" />
